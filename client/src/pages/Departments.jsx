...
export default function Departments() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            if (editing) await api.updateDepartment(editing.id, form)
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-[var(--text-main)]">Departments</h1>
                    <p className="text-[var(--text-muted)] mt-1">Manage corporate departments for {activeEntity?.name || 'this entity'}.</p>
                </div>
                {canEditConfigs && (
                    <button onClick={handleAdd} className="btn-primary w-full sm:w-auto">+ Add Department</button>
                )}
            </div>
                    ...
                    <table className="table-theme w-full">
                        ...
                        <tbody>
                            ...
                            {departments.length === 0 && (
                                <tr>
                                    <td colSpan="3" className="text-center py-8 text-[var(--text-muted)]">No departments found.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
            ...
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
                    <div className="card-base p-6 w-full max-w-md animate-slide-up" style={{ background: 'rgba(15, 23, 42, 0.95)' }}>
                        <h2 className="text-xl font-bold text-[var(--text-main)] mb-6">{editing ? 'Edit' : 'Add'} Department</h2>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-[var(--text-muted)] mb-1.5">Department Name</label>
                                <input
                                    type="text"
                                    required
                                    value={form.name || ''}
