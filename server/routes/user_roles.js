...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db.exec(`SELECT * FROM user_roles ORDER BY name`);
...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        await db.run(`INSERT INTO user_roles (name, description, permissions) VALUES (?, ?, ?)`,
...
router.put('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const db = await getDb();
        const { name, description, permissions } = req.body;
        await db.run(`UPDATE user_roles SET name = ?, description = ?, permissions = ? WHERE id = ?`,
...
router.delete('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const check = await db.exec(`SELECT name FROM user_roles WHERE id = ?`, [req.params.id]);
